...
```{r test1aiii}
pnorm(25.5, 0.25*100, sqrt(100*0.25*0.75))-pnorm(24.5, 0.25*100, sqrt(100*0.25*0.75))
dbinom(25, 100, 0.25)
```
...
```{r test1aiv}
pnorm(19.5, 0.25*100, sqrt(100*0.25*0.75))
pbinom(19, 100, 0.25)
```
...
library(ggplot2)
library(gridExtra)
pois <- data.frame(Outcomes=seq(0,10), Probability=dpois(0:10, 0.05*100))
binom <- data.frame(Outcomes=seq(0,10), Probability=dbinom(0:10, 100, 0.05))

pois_plot <- ggplot(pois, aes(x=Outcomes, y=Probability)) + 
  geom_bar(stat="identity", fill="blue") +
  ggtitle("Poisson Probabilities") +
  theme(legend.position = "none")

binom_plot <- ggplot(binom, aes(x=Outcomes, y=Probability)) + 
  geom_bar(stat="identity", fill="red") +
  ggtitle("Binomial Probabilities") +
  theme(legend.position = "none")

grid.arrange(pois_plot, binom_plot, nrow=1)
...
exp <- round(sum(out*probs), 2)
exp
var <- round(sum((out-exp)^2*probs), 2)
var
...
```{r test1cii}
csum <- data.frame(Outcomes=out, Probabilities=cumsum(probs))
cat("Median: (",median(csum$Outcomes),", ",median(csum$Probabilities),")")
ggplot(data = csum, aes(x = Outcomes, y = Probabilities)) + 
  geom_point(size = 2, color="darkgreen") + 
  ggtitle("Cumulative Probabilities vs. Corresponding Outcomes") +
  ylab("Cumulative Probabilities") +
  geom_point(x=3, y=median(csum$Probabilities), color="lightblue") +
  geom_text(x=3, y=0.8, label="Median = (3, 0.866)")
...
eruption3 <- subset(wait70, wait70$eruptions < 3)
cond_prob <- nrow(eruption3)/nrow(wait70)
cond_prob
...
```{r test2ai}
wait70erupt3 <- subset(faithful, faithful$waiting > 70 & faithful$eruptions < 3.0)
wait70erupt3
ggplot(faithful, aes(x = waiting, y = eruptions)) + 
  geom_point(size = 1, color="red") + 
  ggtitle("Eruption Duration versus Waiting Times") +
  xlab("Waiting Times (minutes)") +
  ylab("Eruption Duration (minutes)") +
  geom_hline(yintercept=3, color="blue") +
  geom_vline(xintercept=70, color="blue") +
  geom_point(x=71, y=2.383, size=1, color="orange") +
  geom_text(x=76, y=2.383, label="(71, 2.383)", color="purple")
   
```

(ii) What does the plot suggest about the relationship between eruption time and waiting time?

***Answer: This plot seems to suggest that there is some correlation between eruption duration and waiting time. We see this since as waiting time increases, the eruption duration seems to tend to increase as well. We can also see two big clusters on the plot, indicating patterns between waiting times and eruption times. This suggests a possible bimodal distribution within the data.    ***
...
ggplot(pairs, aes(x = First, y = Second)) + 
  geom_point(size = 1, color="blue") + 
  ggtitle("Consecutive Pair Waiting Times") +
  xlab("First of Pair Waiting Time (minutes)") +
  ylab("Second of Pair Waiting Time (minutes)")
...
```{r test2c}
cor.test(pairs$First, pairs$Second, alternative="two.sided", method="kendall")
...
boxplot(result1$weight, main="Weights on Diet 1", ylab="Weight", col="red")
boxplot(result3$weight, main="Weights on Diet 3", ylab="Weight", col="blue")
...
```{r test3b}
t.test(result1$weight, result3$weight, alternative="two.sided", var.equal=TRUE)
...
```{r test3c}
time21vstime20 <- data.frame(x=time20, y=time21)
colnames(time21vstime20) <- c("weight20", "weight21")

ggplot(time21vstime20, aes(x=weight20, y = weight21)) + 
  geom_point(size = 1, color="darkblue") + 
  geom_abline(intercept=0, slope=1, color="red") +
  ggtitle("Weight Gain Between Day 20 and Day 21") +
  xlab("Weight at Day 20") +
  ylab("Weight at Day 21")
...
```{r test3d}
avwtgain <- time21vstime20$weight21 - time21vstime20$weight20
...
par(mfrow = c(1, 2))
qqnorm(Nile, col="blue", main="QQ Plot of Flow Data", cex=0.75)
qqline(Nile)
boxplot(Nile, horizontal=FALSE, notch=TRUE, col="blue", main="Boxplot of Flow Data", ylab="Flow")
...
stds <- c(sd(sample1), sd(sample2))
vars <- c(var(sample1), var(sample2))

samplesum <- cbind(means, stds, vars)
colnames(samplesum) <- c("Mean", "Standard Deviation", "Variance")
rownames(samplesum) <- c("Sample1", "Sample2")
samplesum
...
hist(warpbreaks$breaks, main="Loom Warp Breaks", xlab="Warp Breaks per Loom", col = "blue")
abline(v = median_breaks, col = "red")
text(x=35, y=15,"Median = 26", col="black")

number <- factor(ifelse(warpbreaks$breaks < median_breaks, "below", "above"))
warpbreaks <- cbind(warpbreaks, number)
summary(warpbreaks)
conttab <- table(warpbreaks$tension, warpbreaks$number)
conttab
...
```{r test5b}
chisq.test(conttab, correct=FALSE)

#The results of this test show the chi-squared value to be 9.0869 and the P-value to be 0.01064. Since the chi-squared is greater than the p-value, we can conclude that we are able to reject the null hypothesis.
```
...
chisqstat <- ((mar_tbl[1,1]-e11)^2/e11) + ((mar_tbl[1,2]-e12)^2/e12) + ((mar_tbl[2,1]-e21)^2/e21) + ((mar_tbl[2,2]-e22)^2/e22) + ((mar_tbl[3,1]-e31)^2/e31) + ((mar_tbl[3,2]-e32)^2/e32)
cat("Chi-squared statistic: ",chisqstat,"\n")

pval <- pchisq(chisqstat, 2, lower.tail=FALSE)
cat("P-value: ", pval)
...
chisq_function <- function(x) {
  ...
  return(list("chi-squared" = chisq, "degrees of freedom" = df, "p-value" = pval))
}
